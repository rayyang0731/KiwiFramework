...
using EditorUtility = KiwiFramework.Editor.Utility.EditorUtility;
	...
	internal static class RectTransformTools
	{
		...
		public static class GroupTool
		{
			...
			public static void UnGroup()
			{
				...
				{
					SceneView.lastActiveSceneView.ShowNotification(new GUIContent("请选中一个节点"), 1f);
					return;
				}

				if (Selection.gameObjects.Length > 1)
				{
					SceneView.lastActiveSceneView.ShowNotification(new GUIContent("只能选择一个对象"), 1f);
					return;
				}
				...
				{
					SceneView.lastActiveSceneView.ShowNotification(new GUIContent("没有子节点对象"), 1f);
				}
			}
			...
			public static void MakeGroup()
			{
				...
				{
					SceneView.lastActiveSceneView.ShowNotification(new GUIContent("不能跨父节点组合"), 1f);
					return;
				}
				...
				if (rectTrans != null)
				{
					...
					{
						var bound      = EditorUtility.GetBounds(item.gameObject);
