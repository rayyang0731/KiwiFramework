	...
	public class View : Singleton<View>, IView
	{
		...
		public IMediator GetMediator(string mediatorTag)
		{
			if (!_mediatorMap.TryGetValue(mediatorTag, out var mediator))
				Debug.LogError($"[{mediatorTag}]还未注册过.");
			return mediator;
		}
		...
		/// <returns>中介器对象</returns>
		public T GetMediator<T>(string mediatorTag) where T : class, IMediator { return GetMediator(mediatorTag) as T; }
		...
		public bool IsExist(IMediator mediator) { return mediator != null && _mediatorMap.ContainsKey(mediator.Name); }
		...
		/// <param name="mediatorTag">中介器标签</param>
		/// <returns>中介器对象是否已经被注册</returns>
		public bool IsExist(string mediatorTag) { return _mediatorMap.ContainsKey(mediatorTag); }
		...
		public void RemoveMediator(string mediatorTag)
		{
			...
			Debug.Log($"Remove mediator: {mediatorTag}");
		}
		...
		/// </summary>
		/// <param name="mediator">中介器对象</param>
		public void RemoveMediator(IMediator mediator) { RemoveMediator(mediator.Name); }
