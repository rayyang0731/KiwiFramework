...
namespace KiwiFramework.Runtime.UI
{
	...
	public class RawImageStateData
	{
...
#if UNITY_EDITOR
		[SerializeField, DisplayAsString] internal string textureGUID;
		...
		private void OnTextureInspectorInit()
		{
			...
			texture = AssetLoader.Editor.LoadByGUID<Texture>(textureGUID);
		}
		...
		private void OnTextureChanged()
		{
			...
			textureGUID = AssetLoader.Editor.GetGUID(texture);
		}
#endif
		...
		/// 状态所用颜色
		/// </summary>
		[HideLabel, ShowIf(nameof(overrideColor)), HorizontalGroup("Color"), PropertySpace]
		public Color color;
		...
		[LabelText("重载尺寸"), PropertyTooltip("Override size"),
		 HorizontalGroup("Size"), DisableIf(nameof(autoUseNativeSize)),]
		...
		[HideLabel, LabelWidth(60),
		 HorizontalGroup("Size"), ShowIf(nameof(overrideSize))]
		public Vector2 size;
