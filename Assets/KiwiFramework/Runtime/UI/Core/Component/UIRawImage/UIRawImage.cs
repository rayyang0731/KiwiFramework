...
namespace KiwiFramework.Runtime.UI
{
	...
	[AddComponentMenu("KiwiUI/UIRawImage")]
	public partial class UIRawImage : UIMultiStateElement<RawImageStateData>, IGray
	{
		#region RawImage 属性

		[SerializeField, HideInInspector] private RawImage native;
		...
		public Texture texture
		{
			...
			set
			{
				...
				if (Native.texture != null && Application.isPlaying)
					AssetLoader.Unload(Native.texture);
		...
		public override async void SetState(int targetState, bool force = false)
		{
			...
			if (!string.IsNullOrEmpty(data.textureName))
			{
				...
				{
					texture = await AssetLoader.LoadAsync<Texture>(data.textureName);
				}
				else
				{
#if UNITY_EDITOR
					texture = AssetLoader.Editor.LoadByGUID<Texture>(data.textureGUID);
