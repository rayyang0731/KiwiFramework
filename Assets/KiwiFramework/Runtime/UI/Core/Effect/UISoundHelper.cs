...
namespace KiwiFramework.Runtime.UI
{
	...
	public class UISoundHelper
	{
		...
		private struct SoundDesc
		{
			...
			/// </summary>
			[LabelText("点击类型")]
			public UIButton.POINTER_TYPE pointerType;
			...
			public SoundDesc(UIButton.POINTER_TYPE pointerType, string enableSfxPath, string disableSfxPath)
			{
		...
		private UIButton.POINTER_TYPE _pointerType = UIButton.POINTER_TYPE.DOWN;
		...
		private bool TryGet(UIButton.POINTER_TYPE type, out string name)
		{
			name = string.Empty;
		...
		/// 播放声音
		/// </summary>
		public void Play(UIButton.POINTER_TYPE type)
		{
			if (Mute)
				return;
