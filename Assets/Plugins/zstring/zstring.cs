...
public class zstring
{
	...
	private unsafe static void memcpy(char* dest, char* src, int count) { byteCopy((byte*) dest, (byte*) src, count * m_charLen); }
	...
	private unsafe static void memcpy(char* dst, char* src, int length, int src_offset) { memcpy(dst + src_offset, src, length); }
	...
	public class zstring_block : IDisposable
	{
		...
		internal zstring_block(int capacity) { stack = new Stack<zstring>(capacity); }

		internal void push(zstring str) { stack.Push(str); }
	...
	//子字符串替换
	public zstring Replace(string old_value, string new_value) { return internal_replace(this._value, old_value, new_value); }
